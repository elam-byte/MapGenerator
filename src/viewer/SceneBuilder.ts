...
import { sampleArc, roadHalfWidth } from '@shared/geometry';

const ROAD_COLOR = new THREE.Color('#2c2c4a');
const CENTERLINE_COLOR = new THREE.Color('#c8a020');
const LANE_EDGE_COLOR = new THREE.Color('#808090');
...
export class SceneBuilder {
  ...
  build(scene: THREE.Scene, model: MapModel): void {
    ...
    for (const junction of model.junctions) {
      const geo = new THREE.CircleGeometry(3, 16);
      const mat = new THREE.MeshBasicMaterial({ color: JUNCTION_COLOR });
      const mesh = new THREE.Mesh(geo, mat);
      mesh.position.set(junction.x, junction.y, 0.01);
      group.add(mesh);
    }
...
function addArcRoad(group: THREE.Group, road: RoadArc) {
  ...
  if (centerPts.length >= 2) {
    ...
    for (let i = 0; i < centerPts.length - 1; i++) {
      clPositions.push(centerPts[i].x, centerPts[i].y, 0.03, centerPts[i + 1].x, centerPts[i + 1].y, 0.03);
    }
